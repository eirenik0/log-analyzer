...
- Keeps backward compatibility with legacy `[profile.session_prefixes]` by auto-resolving them as a simple two-level fallback.
...
- `generate-config` now emits detected session prefixes as generic `[[sessions.levels]]` entries while preserving template-defined session levels (for example `config/profiles/eyes.toml`).
